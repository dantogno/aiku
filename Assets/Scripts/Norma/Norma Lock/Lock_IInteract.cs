...
public class Lock_IInteract : MonoBehaviour
{
    #region Unity events for when the lock is completed for Norma
    public delegate void DelegateLock();
    public static event DelegateLock UsingPuzzle;
    public static event DelegateLock OnHubUnlocked;
    #endregion
    ...
    private void CheckIfFinishedWithLock()
    {
        switch (thisLock)
        {
            case LockType.NormaPuzzle:
                if (passwordInput == "2821" && Input.GetButtonDown("Vertical") && Input.GetAxis("Vertical") < 0 && knobPlacement == 0) // After it resets, call it a win
                {
                    FinishWithLock();                    
                }
                break;
    ...
    // do something special, depending on which lock it is
        ...
        {
            case LockType.NormaPuzzle:
                if (UsingPuzzle != null) UsingPuzzle();
                break;
            case LockType.HubLock:
                if (OnHubUnlocked != null) OnHubUnlocked();
                break;
            default:
                break;        
        }
